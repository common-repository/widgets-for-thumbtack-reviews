...
class TrustindexWidget_thumbtack extends WP_Widget {
...
{
parent::__construct(
'trustindex_thumbtack_widget',
'Trustindex - Thumbtack reviews widget',
[
'classname' => 'trustindex-widget',
'description' => 'Embed Thumbtack reviews fast and easily into your WordPress site. Increase SEO, trust and sales using Thumbtack reviews.'
]
);
}
...
{
global $wpdb;
global $trustindex_pm_thumbtack;
if ($trustindex_pm_thumbtack->is_enabled()) {
extract($args);
echo $before_widget;
$wasError = false;
...
echo $trustindex_pm_thumbtack->renderWidgetFrontend($instance['ti-widget-ID']);
} else if ($trustindex_pm_thumbtack->is_noreg_linked()) {
echo $trustindex_pm_thumbtack->renderWidgetFrontend();
} else {
echo $trustindex_pm_thumbtack->frontEndErrorForAdmins(sprintf(__("Please fill out <strong>all the required fields</strong> in the <a href='%s'>widget settings</a> page", 'trustindex-plugin'), admin_url('admin.php?page='.$trustindex_pm_thumbtack->get_plugin_slug().'/settings.php')));
}
echo $after_widget;
} else {
}
...
global $wp_version;
global $trustindex_pm_thumbtack;
$tiWidgets = $trustindex_pm_thumbtack->get_trustindex_widgets();
...
<?php if ($trustindex_pm_thumbtack->is_trustindex_connected()): ?>
<?php if ($tiWidgets): ?>
...
<?php endforeach; ?>
<?php else: ?>
<?php echo TrustindexPlugin_thumbtack::get_alertbox('warning',
...
<img src="<?php echo $trustindex_pm_thumbtack->get_plugin_file_url('static/img/help-where-is-id.jpg'); ?>" alt="ID column here: <?php echo 'https://admin.trustindex.io/'; ?>widget" />
</div>
<?php else: ?>
<?php echo TrustindexPlugin_thumbtack::get_alertbox('warning',
...
. sprintf(__("Go to <a href='%s'>plugin setup page</a> to complete the one-step setup guide and enjoy the full functionalization!", 'trustindex-plugin'), admin_url('admin.php?page='.$trustindex_pm_thumbtack->get_plugin_slug().'/settings.php&tab=setup_trustindex'))
); ?>
<?php endif; ?>
</div>
<?php
